...
- [x] 保留当前 hunk 目标位置和 `+-3` 搜索作为首选，避免无必要全文件扫描。
- [x] 局部没有候选时，在当前锁内内容中搜索全文件唯一的 exact/行尾等价 block。
- [x] 全文件恰好一个候选才应用；多个候选返回 typed ambiguous 和有界位置；零候选返回 typed no-match 和当前位置附近证据。
- [x] 不把 `line_trimmed`、空白合并、缩进变换或 block similarity 用作全文件自动 fallback。
- [x] 多 hunk 先在内存中全部定位、验证不重叠并计算最终内容，任一 hunk 失败时不写文件。
- [x] 反向应用后仍报告原 hunk 与实际位置的映射；后一个 hunk 的变化不能让前一个 hunk 错配。
- [x] 保留文件末尾换行和 CRLF/LF 策略；行尾等价不能吞掉 Markdown 双空格等有语义内容。
- [x] 覆盖偏移 4 行、远距离唯一、全文件重复、hunk 重叠、空 context、多 hunk 一处失败和大文件扫描上限。
