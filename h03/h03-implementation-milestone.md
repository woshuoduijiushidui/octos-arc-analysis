...
1. 首版 B 只完成 H03a–e：数据约定、预算内分页、提前保存日志、恢复入口、H02 兼容。
...
5. 可选 C 只实验“在一个原始输出内搜索并返回位置”，不同时增大页、调整压缩阈值或修改推理预算。
...
| M6 | 24 项场景回归、观测与 B 冻结 | M0–M5 |
| M7，可选 | 单一输出内搜索实验 C | 冻结 B |
| M8 | 全量官方任务 A/B 或 A/B/C 对照 | B 通过；参与的 C 通过 |
...
- [ ] 若当前主线已有 H02 C，保留代码，默认关闭 `OCTOS_FILE_READ_RETAINED_RECEIPTS` 以减少变量；若既有采用决策要求开启，则 A/B/C 全部保持开启。
...
| B | 从 `A_SHA` 开始，代码分支建议 `feat/output-recovery` | H03a–e，固定页预算与恢复行为 | M6 记录 `B_SHA` |
| C，可选 | 从冻结 `B_SHA` 创建 `feat/output-search` | 只增加输出内有界搜索 | M7 记录 `C_SHA` |
...
- [ ] 单一 C 开关默认关闭；只在决定执行 M7 时添加。未开展 C 不留空实现和死开关。
...
## 10. Milestone M6：观测、确定性回归与 B 冻结
...
下表全部针对 B；已在前置 Milestone 完成的测试直接复用，不再重复写同样的单测。数字是清单 ID，不是应凑齐的测试函数数量。
...
- [ ] 审查 `A_SHA..HEAD`，没有混入 C、官方输入变化或无关重构。
- [ ] 本地提交并冻结干净 `B_SHA`，保存实际二进制哈希和有效策略参数；证据文件建议 `h03-m6-b-variant-freeze.md`。

**完成条件：**无静默缺口、错误游标、错误完整性声明和 H02 错误授权；所有容量和失败路径有界。B 具备实验条件，但尚不宣称提高官方正确率或减少 token。

## 11. Milestone M7：可选 C，只增加输出内搜索

**依赖：**冻结后的 B。B 的实现完成不以 C 必须开展为前提。

- [ ] 若选择开展，从 `B_SHA` 创建独立分支；若不开展，将 M7 标为“未开展，可选”，M8 执行 A/B。
...
- [ ] page/执行/存储预算、压缩、H02、模型和外层流程与 B 完全一致；C 只改变定位方式及其必要 schema。
- [ ] 验证无匹配、多匹配、跨块匹配、超限续查、partial artifact、取消与越权。
- [ ] 关闭 C 时行为与 B 等价；审查 diff 后冻结 `C_SHA`，记录新增工具定义的输入 token 成本。

**完成条件：**C 相对 B 只有一项可归因变化，且保持全部 B 正确性约束。更大页策略另立后续实验，不混入 C。

## 12. Milestone M8：固定官方任务的 A/B 或 A/B/C 对照

### 12.1 开跑条件

- [ ] B 的全部确定性门槛通过；参与比较的 C 同样通过。先确认当前执行范围允许真实模型实验。
...
- [ ] A/B/C 的 requirements、测试、模板、模型、reasoning、生成档位、session scope、工具权限和请求/修复预算一致；差异仅限所属 H03 功能和必要恢复工具。
- [ ] 时间不作为评分指标；既有超时/墙钟截止仍会影响结果，三组保持相同设置并记录，不能只给 B 延长执行预算。
...
- [ ] `MAIN_SHA/A_SHA/B_SHA/C_SHA`、实际运行 SHA、git dirty、二进制路径与哈希。
- [ ] 任务 ID、官方输入/测试/模板哈希、模型及非敏感 endpoint 标识、repetition/run_order。
- [ ] H01/H02/H03/C 开关、有效页/存储/compaction policy、工具列表与权限、session scope。
...
- [ ] 分别报告 B−A 的恢复链整体效果、C−B 的搜索边际效果；不将依赖移植或 C 合并成一个不可归因百分比。
...
- [ ] 汇总 B/C 是否满足确定性与官方测试门槛；C 不优于 B 时保持 B，不因已写代码强行采用。
...
每步运行与改动相关的测试；M6 再汇总跨层回归。先用 `-- --list` 核对测试实际存在和过滤范围；下面是候选命令，不是已运行记录。
...
建议提交按 M0→M6 逐步进行；一个 Milestone 过大时拆成保持绿色的小提交。提交信息写实际行为，如“保证文件页与续读提示完整进入模型”“保存并恢复同一次命令的输出”。不把内部代号当成变更说明。
