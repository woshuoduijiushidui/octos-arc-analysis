...
本节描述调研得出的改动边界；开发顺序与验收见 [H03 实施 Milestone](./h03-implementation-milestone.md)。当前采用 A/B 两组，将输出内有界搜索纳入完整 B 方案。
...
| H03f | 输出内有界搜索并按位置读取；更大页和智能片段排序暂缓 | 有望减少盲目翻页；增加搜索 schema 和调用成本，净收益待测 | **有界搜索纳入 B** |
...
| 输出内搜索：无匹配、多匹配、跨块匹配、扫描上限 | 返回位置准确；部分扫描不冒充全文无匹配 |
| 搜索后按位置恢复，取消和越权搜索 | 最终请求含目标片段；与恢复共用预算、权限和清洗约束 |

前两项建议使用假 provider 捕获真正发出的请求，并把所有返回页按源范围重组后与固定原文比较。只验证单个分页 helper，会漏掉目前最关键的二次截断。

### 8.2 固定官方输入，比较完整方案的 A/B 两组

| 组别 | 策略 | 要回答的问题 |
| --- | --- | --- |
| A | 固定共同基线，保留现有输出行为 | 当前有多少工具页/失败输出实际被截断 |
| B | A + H03a–e + H03f 中的输出内有界搜索，使用固定有效页预算 | 完整优化方案是否改善通过表现和全任务 token |

两组使用相同 H01/H02 开关和提交；不能把 H02 新增收益混算给 H03。不设独立 C 组，恢复与搜索分阶段开发、分别做确定性测试，最终一起进入 B 的官方任务对照。

此设计能够判断完整方案的净收益，但不能单独证明搜索贡献了多少。只有未来确实需要定位单项收益时才补充消融实验；它不是本轮的必做项。更大页策略、智能片段排序和压缩阈值调整不进入本轮。
...
H03 值得做，优先顺序应是：**真实范围与完整性描述 → 最终预算内分页 → 首次裁剪前保存日志 → 接通同一份原文的恢复 → 校验 H02 凭据 → 增加输出内有界搜索**。最终将完整 B 与共同底座 A 比较。
